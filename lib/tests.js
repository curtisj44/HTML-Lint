(function (htmlLint) {
  const tests = {
    ...
    'desc:contains("Created with Sketch")' : {
      'label': 'Inaccurate SVG `desc` left in place from Sketch'
    },
    ...
    'iframe:not([title])': {
      'label': '`iframe` missing `title`',
      // details: [{
      //  quote: 'Iframes are named via their title attribute.',
      //  source: 'https://www.mediacurrent.com/blog/accessible-names-label-all-things-part-1'
      // }]
    },
    ...
    'meta[http-equiv="X-UA-Compatible"]': {
      ...
      // details: [{
      //  source: 'https://msdn.microsoft.com/en-us/library/jj676915%28v=vs.85%29.aspx?f=255&MSPPError=-2147217396'
      // }]
    },
    'meta[name="twitter:image:src"]': {
      label: '`twitter:image:src` isn’t correct'
      // details: [{
    ...
    'meta[name="viewport"][content*="minimal-ui"]': {
      ...
      // details: [{
      //  source: 'https://developer.apple.com/library/ios/releasenotes/General/RN-iOSSDK-8.0/'
      // }]
    },
    'nav:not([role])': {
    ...
    //
    // Attributes without elements

    '[align]': {
      'label': '`align` attribute is obsolete'
    },
    '[alink]': {
      'label': 'Bad attribute: `alink`'
    },
    '[allowfullscreen], [mozallowfullscreen], [webkitallowfullscreen]': {
      'label': '`allowfullscreen` is deprecated'
      // details: [
      //  quote: 'This attribute is considered a legacy attribute and redefined as `allow="fullscreen"`.',
      //  source: 'https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe#Attributes'
      // }]
    },
    '[allowpaymentrequest]': {
      'label': '`allowpaymentrequest` is deprecated'
      // details: [
      //  quote: 'This attribute is considered a legacy attribute and redefined as `allow="payment"`.',
      //  source: 'https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe#Attributes'
      // }]
    },
    '[allowtransparency]': {
      'label': '`allowtransparency` attribute is obsolete'
    },
    '[aria-hidden]:not([aria-hidden="true"])': {
      'label': '`aria-hidden` attribute is present, but not equal to "true"'
      // details: [{
      //  source: 'https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/ARIA_Techniques/Using_the_aria-hidden_attribute'
      // },{
      //  quote: 'At the time of this writing, `aria-hidden="false"` is known to work inconsistently in browsers. As future implementations improve, use caution and test thoroughly before relying on this approach.',
    ...
    '[frameborder]': {
      'label': '`frameborder` attribute is obsolete'
    },
    '[halign]': {
      'label': 'Invalid attribute: `halign`'
    },
    '[hspace]': {
      'label': '`hspace` attribute is obsolete'
    },
    '[id=""]': {
      'label': '`id=""`'
    },
    ...
    '[marginheight]': {
      'label': '`marginheight` attribute is obsolete'
    },
    '[marginwidth]': {
      'label': '`marginwidth` attribute is obsolete'
    },
    '[name=""]': {
      'label': '`name=""`'
    },
    ...
    '[scrolling]': {
      'label': '`scrolling` attribute is obsolete'
    },
    '[shape]': {
    ...
    '[src=""]': {
      'label': '`src` attribute is empty'
    },
    '[src*="javascript:"]': {
      'label': '`[src*="javascript:"]`'
    },
    '[srcset=""]': {
      'label': '`srcset` attribute is empty'
    },
    '[style*="background"]': {
    ...
    '[tabindex]': {
      ...
      // details: [{
      //  quote: 'Be careful with `tabindex`. Especially, avoid using a value greater than 0',
      //  source: [
    ...
    '[vspace]': {
      'label': '`vspace` attribute is obsolete'
    },
